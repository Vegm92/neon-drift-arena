...
`index.html` at the root is the marketing page: a muted autoplay loop of `public/trailer.mp4` (a 27 s cut of real bots matches, poster `public/trailer.jpg`), one PLAY IN BROWSER button pointing at `/play/`, four feature rows and the OG card `public/og.jpg`. Every string on it lives in `strings.json`; the `site-strings` hook in `vite.config.js` substitutes `{{key}}` placeholders in dev and build. `npm run build` emits the page, the game and the pad into `dist/`. `npm run deploy` builds, stages `dist/` behind the Node image in `deploy/` and uploads it with the Railway CLI to the `site` service of the `neon-drift-arena` project (https://site-production-a98b.up.railway.app). `deploy/server.mjs` serves those files gzipped and carries the phone-pad relay on `/relay`, so the pad works on the deployed build too.
