...
The keyboard column is the default map, not a fixed one: SETTINGS > KEY BINDINGS lists every keyboard action and rebinds it. Fire on a row captures the next key you press and makes it the only key for that action, `▶` captures a key and adds it as an extra one, `◀` drops the last extra, and Backspace cancels a capture. A key that is another action's last remaining key is refused, and RESET KEYS puts the whole map back. Whatever the map says, Enter and Backspace always still reach the menus, so a rebind can never lock you out. Esc is not a game key at all: CrazyGames takes it to leave fullscreen, so the game ignores it and never offers it. Gamepad buttons in every legend are drawn as the button - a round green A, a round red B, a bumper-shaped RT - so a legend never reads "B" as a keyboard key. Every key name the game prints — the lobby legend, the settings hints, the HOW TO PLAY card — comes from the map, and from your keyboard layout, so an AZERTY board reads Z Q S D.
...
SETTINGS lives in the lobby: a READY player moves down to the SETTINGS entry and confirms. The panel is a single controller-driven list - up/down moves, left/right adjusts, A/Space toggles, B / Backspace goes back - covering controller slots, SWAP STICKS, KEY BINDINGS, ARENA and AUDIO. The TUNING rows (every tunable in `Cfg`, plus RESET) are a developer tool and only show with `?dev=1` in the URL.

## The lobby

The lobby opens pre-seeded: the keyboard holds P1 and a bot holds P2, so one player can press Start and be in a match immediately. That keyboard seat is only a guess until the keyboard acts in the lobby: a gamepad or phone that presses Fire or A first takes P1 over, so playing on a pad needs no leaving the keyboard first. Once the keyboard has touched the lobby, its seat is firm and every other device presses Fire or A to claim one of the remaining slots (explicit slot from SETTINGS, else the first free one). Your CrazyGames username sits on this machine's first human seat, whichever device holds it, and follows the remaining local player if that seat empties; every other local seat shows the device's own name (the gamepad's model), a phone or joined desktop shows the name it brings, and a bot's card reads BOT. Up on your own card types a new name for a keyboard or gamepad seat - it sticks until you clear it, and an empty name restores the default; a phone or joined desktop keeps its CrazyGames name so friends recognise it. On your ship card, left/right picks your colour - CYAN / MAGENTA / LIME / AMBER, never two players on the same one - or your side in TEAMS mode, Fire confirms you as READY, and Back (Backspace, or B on a pad) un-confirms, then leaves. Down moves you from your card onto the shared menu row below the slots, where left/right moves the cursor and Up (or Back) returns to your card; the cursor pulses and shows the tags of the players sitting on it, and a card whose player went down dims with a ▼. MODE cycles FREE FOR ALL, TEAMS, PRACTICE and RACE (any change resets everyone to unconfirmed), START launches, SETTINGS opens the panel. A device whose saved slot is already taken joins the first free slot instead. START only lights up with 2+ players, everyone READY, and - in TEAMS - both sides filled; Start (Enter) launches from anywhere once that holds. Teammates share a colour, do not damage each other, and win together. Slots not in the lobby are ignored during the match. Every launch opens with a 5 s countdown while the camera flies low over the four spawn rings - each lit in its ship's colour with the player's tag floating above - and resuming from pause counts 3-2-1 from the normal view. Start during play opens the pause menu (RESUME / RESTART MATCH / QUIT TO LOBBY); the result screen offers REMATCH / QUIT TO LOBBY. Menus navigate with stick, d-pad or W/S, confirm with A / Fire / Start, back with B / Backspace.

The lobby's menu row holds MODE, ARENA (the five layouts plus RANDOM, rolled at every launch and rematch), MUTATOR (NONE, RAILS ONLY - every crate is a railgun, TURBO - half again as fast in every axis, ICE - a quarter of the drag), ADD BOT (fills the first empty slot with a bot that hunts, dodges rock, breaks off a ram it would lose, refuses to trade itself away in a head-on pass, and goes shopping for crates and pads when there is nobody left to fight; when the lobby is full it turns into CLEAR BOTS), SETTINGS and START. Wins carry across rematches as ★ on the cards and on the result screen; the first to 5 takes the series and the tally resets, as it does whenever the roster or mode changes. On the result screen Back leaves your slot and a device that is not in yet joins with Fire, so a pad can change hands before REMATCH. Mid-match drop-in works too: a pad, phone or the keyboard that is not in the game presses Start or Fire and its ship spawns straight away with full stocks - into a free slot, or in place of a bot when the lobby is full - with a "JOINS THE FIGHT" shout. No menu, no restart. SETTINGS > ARENA has a CATCH-UP toggle (on by default): the ship with the fewest stocks respawns with a full boost bar and a shield. Beside it a REDUCE FLASHING toggle (off by default) kills every flash - the takedown and GO screen washes, the white flash disc on a takedown and the bloom surge - and a SCREEN SHAKE toggle (on by default) covers the motion, the camera jolt and the HUD panel shake, so flashing can be turned down without giving up the shake.
...
- Asteroids block bullets and bounce ships, and how badly you come off depends on how you hit. Square on, the rock stops you dead and takes the controls for up to two thirds of a second, but your nose stays where you left it. Clip one at an angle and you keep more of your speed and come away tumbling, with the spin bleeding off over the next second or so. None of it is canned — the bounce, the tumble and how long you are out of control all fall out of the actual contact, so the same rock never punishes two approaches the same way.
...
MODE → RACE turns the ARENA row into a track picker - GRAND PRIX (a full-width circuit with a dozen corners), HAIRPIN (two tight right-handers and an S between two long straights) and INFINITY (a figure of eight that crosses itself in the middle) - and RANDOM rolls among tracks. The intro fly-by follows the road instead of the spawn rings. Tracks sit in a bigger arena than the battles (1800 half-width against 1350) and the road is smoothed through every corner, drawn as a ribbon of two neon edges with a yellow start line; every few bends a violet portal ring sits on the road as a checkpoint, the finish ring is yellow. You must fly through the portals in order, so cutting the infield gains nothing; the rings you have to take next glow, and when nothing is threatening you the arc around your ship turns violet and points at your next portal. The panel shows `LAP n/3` instead of stocks and the clock counts up. RACE runs at Mario Kart pace: everything - turn, thrust, top speed - is 25% faster than a battle, and a boost is a real burst past top speed rather than a quicker way to reach it, tailing off over a fraction of a second when the bar runs dry. Ten boost-pad clusters line the road; in RACE a pad refills 45 boost, kicks the ship along its nose the moment it is touched and is back two seconds later. Crates come in item rows: three boxes side by side across the whole road at four spots per track, so the ship behind the leader has a box of its own, and a taken box reappears in place three seconds later. What a box holds depends on where you are: the leader draws from the defensive half (ZAPPER, CONTACT MINES, TIME BUBBLE, TRACTOR), the last ship from the hunting half (MISSILE, REPULSOR, TRACTOR, ZAPPER) plus a boost top-up worth a full crate, everyone in between from the whole race arsenal; the further back you are, the more boost a box refills. The blaster is off - RACE is about piloting, not shooting - and crates only hand out ZAPPER, CONTACT MINES (a race-only variant: the mine sits still where you drop it and blasts the first enemy that touches it), a single MISSILE (the race-only seeker: it flies straight, no homing), the REPULSOR (a shell in RACE: a much harder shove and a short spin-out, with the shove bleeding off instead of being cut at top speed), TRACTOR and TIME BUBBLE; any hit that would hurt stuns for a second instead, so hulls never drop and the only way to wreck is to leave the arena. Nobody loses stocks: a wreck respawns after 3 s at the last checkpoint you crossed, facing the next one. No match clock, no sudden death, no wormholes or black holes. Crossing the start line after the last lap parks the ship, shouts its place and starts a 20 s grace period; the race ends when everyone is in or the grace runs out, and the first across the line wins. `laps`, `gateRadius`, `raceGrace`, `raceSpeed`, `raceBoostTop`, `raceBoostFade`, `racePadRefill`, `racePadKick`, `raceCrateBoost`, `racePulseForce` and `racePulseStun` live in SETTINGS > TUNING (`?dev=1`).

## Practice mode

PRACTICE starts as soon as one player from any device - keyboard, gamepad or phone - has joined and pressed Start (no READY needed), and drops a passive target bot into the first free slot. Everyone lines up on the left facing the target, with a crate above the centre and a dormant mine below. Nobody loses stocks, so the range never ends until QUIT TO LOBBY. The crate reappears a second after every grab and hands out the weapons in order - RAILGUN, MAG MINES, SEEKERS, REPULSOR, ZAPPER, TRACTOR, BARRIER, SENTRY, TIME BUBBLE - so a pad can cycle through all of them. With a keyboard in the game, `0`–`9` arm it directly (0 = BLASTER), `K` kills the target, `G` turns the keyboard player into a launcher, `T` jumps to sudden death and `R` resets the stage.
