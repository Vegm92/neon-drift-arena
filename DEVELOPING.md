...
- **Identity.** `Menu.initUser` keeps the machine's username in `Menu.localName` from `getUser` and updates it live through `addAuthListener`; the lobby shows it on `Menu.localSeat`, the first seat a local device holds, and a typed rename on a local seat wins over it. SETTINGS shows a SIGN IN row only while `Settings.isGuest`. The data module holds one key, `nda-ad-seen`, which skips a new player's first midgame ad; there is no player progress in V1.
...
The lobby carries a COPY INVITE LINK panel next to the phone QR. The link is the SDK's own: `updateRoom` asks `sdk.game.inviteLink` for it and caches it, and `CrazyGames.inviteLink ()` hands it to the panel. Off CrazyGames there is no SDK to ask, so the panel falls back to `<this page>?room=<code>`, the plain query parameter `CrazyGames.getInviteRoom ()` already reads, and a friend on any URL still joins the same room.

The lobby also carries ROOM CHAT, top right, for the desktops sharing a room - the people an invite link brought in, not the phones on the same couch. The host owns the one log: it appends its own lines and the `nda:chat` lines a peer sends, then mirrors the whole log to every peer inside `nda:state`. The panel is its own `#chat` element rather than part of the mirrored menu HTML, because a peer's half-typed line has to survive the host's next menu refresh. Messages are escaped before they are drawn, capped at `Cfg.chatMax` characters, and only the last `Cfg.chatKeep` are kept. `T` focuses the box, Enter sends. The panel follows the menu's own class, so it is up in the lobby and gone for the round; `sdk.game.settings.disableChat` hides it outright and drops anything that arrives while it is set.

`node mp-check.cjs` drives two desktops through a room with Playwright against a built site on `localhost:8791` (`deploy/server.mjs` with `dist` copied in as `site`): roles, the send rate, the chat round trip both ways, escaping, `disableChat`, the room reopening between rounds, and host promotion.
