...
LAN mirror clients (`nda:state`, host-authoritative) ride the Vite dev socket only. The host sends `nda:state` at a fixed `Cfg.netStateMs` (100 ms) tick instead of every render frame, and only includes the lobby menu HTML when it changed or a second has passed since the last copy went out — a full-combat `nda:state` payload runs under 10 KB, so the tick keeps the sustained stream under ~100 KB/s over the relay. Losing the host mid-match ends the match for every mirror and returns them to the lobby rather than trying to migrate the host role — see `docs/adr/0001-online-multiplayer-host-loss-and-lobby-size.md`.
