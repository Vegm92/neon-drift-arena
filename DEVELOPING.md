...
LAN mirror clients (`nda:state`, host-authoritative) ride the Vite dev socket only. The menu HTML was already deduped by content plus a 1s heartbeat before the WAN work; that work added the `Cfg.netStateMs` (100 ms) send tick on top, capping the rest of the payload that used to go out every render frame. A full-combat `nda:state` payload is estimated at ~7.5-9.3 KB, which puts the sustained stream at an estimated ~75-93 KB/s over the relay — a synthetic estimate from payload size times send rate, not a captured measurement. Losing the host mid-match ends the match for every mirror and returns them to the lobby rather than trying to migrate the host role — see `docs/adr/0001-online-multiplayer-host-loss-and-lobby-size.md`.
