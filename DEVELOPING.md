...
- **Progress and identity.** `Menu.loadProgress` / `saveProgress` keep `nda-high-score` and `nda-matches-played` in the data module; `Menu.initUser` keeps the machine's username in `Menu.localName` from `getUser` and updates it live through `addAuthListener`; the lobby shows it on `Menu.localSeat`, the first seat a local device holds, and a typed rename on a local seat wins over it. SETTINGS shows a SIGN IN row only while `Settings.isGuest`.
