...
- No internet networking, no inventories; LAN phone pads and LAN mirror clients (`nda:state`, host-authoritative) ride the Vite dev socket (`pad.html`, dev server only). UI is the lobby overlay (`Menu.fs`), the SETTINGS panel, the four corner panels and one banner.
